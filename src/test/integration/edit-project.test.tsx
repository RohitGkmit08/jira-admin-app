...
describe('edit project dialog flow', () => {
  ...
  const submitEditForm = async (name: string, beforeSave?: () => void) => {
    const dialog = await openEditDialog();
    const dialogUtils = within(dialog);

    const input = dialogUtils.getByLabelText(/project name/i);
    ...
    await user.click(dialogUtils.getByRole('button', { name: /^save$/i }));
  };
  ...
  test('calls updateProject API with correct payload', async () => {
    ...
    await submitEditForm('Test Project updated', () => {
  ...
  test('save button is disabled and loader shows while request pending', async () => {
    ...
    await user.clear(input);
    await user.type(input, 'Test Project updated');
